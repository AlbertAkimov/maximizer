...
variant_t Component::toStlVariant(tVariant src) {
    ...
    switch (src.vt) {
        ...
        case VTYPE_PWSTR: //std::string
            return  std::u16string((char16_t*)src.pwstrVal, src.wstrLen);
...
void Component::storeVariable(const variant_t &src, tVariant &dst) {
    ...
    std::visit(overloaded{
            ...
            [&](const std::u16string &v) { storeVariable(v, dst); },
...
void Component::storeVariable(const std::u16string &src, tVariant &dst) {

    dst.vt = VTYPE_NULL;
    storeVariable(src, &dst.pwstrVal);
    dst.wstrLen = static_cast<uint32_t>(src.length());
}
